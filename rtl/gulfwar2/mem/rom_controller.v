...
(
    ...
    input  [1:0]  sprite_rom_base_sel,
    output [31:0] sprite_rom_data,
    output        sprite_rom_data_valid,
    output reg    sprite_download_seen,
    output reg    sprite_download_nonzero,
...
reg [1:0]  sprite_rom_base_sel_q;
reg [22:0] sprite_rom_pending_addr;
reg [22:0] sprite_rom_cache_addr;
reg [31:0] sprite_rom_cache_data;

wire [22:0] sprite_rom_base_addr =
    (sprite_rom_base_sel == 2'd1) ? 23'h030800 : // byte base 0x0c2000
    (sprite_rom_base_sel == 2'd2) ? 23'h030800 : // byte base 0x0c2000
    (sprite_rom_base_sel == 2'd3) ? 23'h030800 : // byte base 0x0c2000
                                    23'h030800;  // byte base 0x0c2000
...
always @(posedge clk) begin
    if (reset && !ioctl_download && !download_pending) begin
        ...
        sprite_download_seen <= 1'b0;
        sprite_download_nonzero <= 1'b0;
    end else begin
        ...
        if (download_byte && !download_pending) begin
            if ((ioctl_index == 16'd0) && (ioctl_addr >= 27'h00c2000) && (ioctl_addr < 27'h0102000)) begin
                sprite_download_seen <= 1'b1;
                sprite_download_nonzero <= sprite_download_nonzero | (ioctl_data != 8'd0);
            end
...
always @(posedge clk, posedge reset) begin
    if (reset) begin
        ...
        sprite_rom_base_sel_q <= 2'd0;
        sprite_rom_pending_addr <= 23'd0;
        sprite_rom_cache_addr <= 23'd0;
        sprite_rom_cache_data <= 32'd0;
    end else begin
        if (sprite_rom_base_sel_q != sprite_rom_base_sel) begin
            sprite_rom_full <= 1'b0;
            sprite_rom_pending <= 1'b0;
            sprite_rom_base_sel_q <= sprite_rom_base_sel;
        end else if (sprite_rom_ctrl_ack) begin
...
wire sound_rom_w = ioctl_wr &&
                   (ioctl_addr >= 27'h0040000) && (ioctl_addr < 27'h0048000);

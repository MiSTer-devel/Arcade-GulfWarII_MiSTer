...
//	bit  [15: 0] ROM_Q;
	...
	wire        ACC_Z_W = ~|ACC;
	wire        ACC_N_W = ACC[31];
	wire        AR_Z_W  = ~|AR[ST.ARP][8:0];
	wire        BR_COND_TRUE =
		(IC[11:8] == 4'h4) ? !AR_Z_W :
		(IC[11:8] == 4'h5) ? ST.OV :
		(IC[11:8] == 4'h6) ? BIO :
		(IC[11:8] == 4'hA) ? ACC_N_W :
		(IC[11:8] == 4'hB) ? (ACC_N_W || ACC_Z_W) :
		(IC[11:8] == 4'hC) ? (!ACC_N_W && !ACC_Z_W) :
		(IC[11:8] == 4'hD) ? (!ACC_N_W || ACC_Z_W) :
		(IC[11:8] == 4'hE) ? !ACC_Z_W :
		(IC[11:8] == 4'hF) ? ACC_Z_W :
		                      1'b1;
	wire        BR_CONDITIONAL =
		(IC[15:12] == 4'hF) &&
		((IC[11:8] == 4'h4) ||
		 ...
		 (IC[11:8] == 4'h6) ||
		 (IC[11:8] >= 4'hA));
	wire        FAST_BRANCH_SKIP = (STATE == 2'd0) && BR_CONDITIONAL && !BR_COND_TRUE;
	wire        FAST_BANZ_DEC = FAST_BRANCH_SKIP && (IC[11:8] == 4'h4);
...
//	);
	...
	wire [ 7: 0] RAM_WA = !DECI.RAMN ? RAM_A : RAM_A + 8'd1;
	bit  [15: 0] RAM_Q;
...
(
    ...
    .data_b ( DBO ),
    .q_b ( ),
    .wren_b ( DECI.RAMW & EN & CE_R )
);
	
		wire [11: 0] PC_NEXT = PC + 12'd1;
		wire         IRQ_DEFER = (IC == 16'h7F82) || (IC[15:8] == 8'h6D) || ((IC[15:13] == 3'b100));
	
	assign AR_N = DECI.ARW || DECI.ARR ? IC[8] : ST.ARP;
	always_comb begin
		bit [8:0] AR_LOW_NEXT;
		case (IC[5:4])
			2'b10: AR_LOW_NEXT = AR[ST.ARP][8:0] + 9'd1;
			2'b01: AR_LOW_NEXT = AR[ST.ARP][8:0] - 9'd1;
			default: AR_LOW_NEXT = AR[ST.ARP][8:0];
		endcase
		AR_NEXT <= { AR[ST.ARP][15:9], AR_LOW_NEXT };
		AR_BANZ_NEXT <= { AR[ST.ARP][15:9], AR[ST.ARP][8:0] - 9'd1 };
		AR_DOUT <= (DECI.ARM && (IC[8] == ST.ARP)) ? AR_NEXT : AR[IC[8]];
	end
	...
	always_comb begin
		...
		case (DECI.DOS)
			...
			DOS_NPC: DBO <= PC_NEXT;
			DOS_DM:  DBO <= RAM_Q;
			DOS_PORT:DBO <= DECI.TBLR ? ROM_Q : DI;
			default: DBO <= '0;
		endcase
	...
	always_comb begin
		...
		ABS_RES = (ACC == 32'h8000_0000 && ST.OVM) ? 32'h7fff_ffff :
		          (ACC[31] ? (~ACC + 32'd1) : ACC);
	...
	always @(posedge CLK or negedge RST_N) begin		
		if (!RST_N) begin
			...
			if (DECI.ARD || FAST_BANZ_DEC) begin
				AR[ST.ARP] <= AR_BANZ_NEXT;
			end
			if (DECI.ARW) begin
				AR[IC[8]] <= DBI;
			end
			...
			endcase
					if (DECI.LST && (IC[15:8] == 8'hF5) && ST.OV) begin
						ST.OV <= 0;
					end
				end
			end
	...
	always @(posedge CLK or negedge RST_N) begin		
		...
		if (!RST_N) begin
			...
			if (EN) begin
				...
				if (!STATE) begin
					if (INT_PEND && !ST.INTM && !IRQ_DEFER) begin
						IC <= 16'h7F80; // fetch bubble
						STACK[0] <= PC[11:0];
						STACK[1] <= STACK[0];
						STACK[2] <= STACK[1];
						STACK[3] <= STACK[2];
			...
			if (EN) begin
				if (IRQ_SERV) begin
					...
					case (IC[11:8])
						...
						4'hC: COND = !ACC_N && !ACC_Z; //BGZ
						4'hD: COND = !ACC_N || ACC_Z; //BGEZ
						4'hE: COND = !ACC_Z; //BNZ
						4'hF: COND =  ACC_Z; //BZ
						default: COND = 1;
					endcase
				if (FAST_BRANCH_SKIP) begin
					PC <= PC + 12'd2;
				end else begin
...
//	} PCUpdate_t; 
				...
				STATE <= FAST_BRANCH_SKIP ? 2'd0 : (STATE + 2'd1);
				if (DECI.LST || FAST_BRANCH_SKIP) begin
					STATE <= '0;
				end
